	...
	<view>
		...
		<view class="container">
			<view class="info-wrap">
				<u-avatar class="avatar" :src="userInfo.face" :show-level='userInfo.type == 1' level-bg-color="#8072f3"
					size="130"></u-avatar>
				<view class="user-style">
					<view class="username">{{userInfo.nickName}}
						<text class="iconfont icon-nan kong" v-if="userInfo.sex=='男'"></text>
						<text class="iconfont icon-nv kong" v-else-if="userInfo.sex=='女'"></text>
					</view>
			...
			<view>
				...
				<postList :list="postList" :loadStatus="loadStatus"></postList>
			</view>
		</view>
		...
		methods: {
          ...
          // else {
			...
			getPostList() {
				...
				getPostList(postListForm).then((res) => {

					this.postList = this.postList.concat(res.data.result.data);
					if (res.data.result.current_page === res.data.result.last_page || res.data.result.last_page ===
						0) {
			...
			getUserInfo() {
				...
				userInfoById(info).then((res) => {
					if (res.data.code == 0) {
						this.userInfo = res.data.result;
						if (res.data.result.sex === 1) {
							this.userInfo.sex = '男'
						} else if (res.data.result.sex === 0) {
							this.userInfo.sex = '女'
						} else {
