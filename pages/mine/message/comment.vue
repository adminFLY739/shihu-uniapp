<template>
	<view>
		<view>
			<!-- 评论转发消息 -->
			<view class="message">
				<block v-for="(item, index) in commentMessageList" :key="item.id">
					<view class="message-item">
						<view class="messageList-item">
							<view class="center">
								...
								<!-- 评论内容 -->
								<rich-text class="content" :nodes="parseContent(item.content)"></rich-text>
	...
	export default {
		...
		methods: {
			...
			getcommentMessageList() {
				this.loadStatus = "loading";
				getCommentMessage(this.params.pageNumber).then((res) => {
					if (this.params.pageNumber === 1) {
						this.commentMessageList = res.data.result.data;
					} else {
						this.commentMessageList = this.commentMessageList.concat(res.data.result.data);
					}
					console.log("commentMessageList:", this.commentMessageList);
			...
			jumpToPost(postId, messageId, isRead) {
				//更新消息为已读
				if (!isRead) {
					updateCommentMessagesAsRead([messageId]).then(() => {});
				}
			...
			allRead() {
				let messageIds = [];
				this.commentMessageList.forEach((item) => {
					if (!item.isRead) {
				...
				if (messageIds.length !== 0) {
					updateCommentMessagesAsRead(messageIds).then(() => {
						this.getcommentMessageList();
					})
				}
			}
