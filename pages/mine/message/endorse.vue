<template>
	<view>
		<view>
			<!-- 赞同喜欢消息 -->
			<view class="message">
				<block v-for="(item, index) in endorseMessageList" :key="item.id">
					<view class="message-item">
						<view class="messageList-item">
							<view class="center">
								...
								<!-- 评论内容 -->
								<text class="content">赞同了你的回答</text>
...
<script>
	...
	export default {
		...
		methods: {
			getEndorseMessageList() {
				this.loadStatus = "loading";
				getEndorseMessage(this.params.pageNumber).then((res) => {
					if (this.params.pageNumber === 1) {
						this.endorseMessageList = res.data.result.data;
					} else {
						this.endorseMessageList = this.endorseMessageList.concat(res.data.result.data);
					}
					console.log("endorseMessageList:", this.endorseMessageList);
			...
			jumpToPost(postId, messageId, isRead) {
				//更新消息为已读
				if (!isRead) {
					updateEndorseMessagesAsRead([messageId]).then(() => {});
				}
			...
			allRead() {
				let messageIds = [];
				this.endorseMessageList.forEach((item) => {
					if (!item.isRead) {
				...
				if (messageIds.length !== 0) {
					updateEndorseMessagesAsRead(messageIds).then(() => {
						this.getEndorseMessageList();
					})
				}
			}
