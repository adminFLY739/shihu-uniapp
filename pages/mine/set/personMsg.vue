<template>
	<view class="person-msg">
		...
		<button class="head c-content" open-type="chooseAvatar" @chooseavatar="onChooseAvatar">
			<image :src="form.face || '/static/missing-face.png'" mode=""></image>
			<view>点击修改头像</view>
		</button>
		...
		<u-form :model="form" ref="uForm" class="form">
			<u-form-item label="昵称" label-width="150">
				<u-input v-model="form.nickName" placeholder="请输入昵称" type="nickname" @input="inputNickname"/>
			</u-form-item>
		...
		data() {
			return {
				...
				form: {
					...
					___path: storage.getUserInfo().region === "undefined" ? "" : storage.getUserInfo().region,
				},
		...
		methods: {
			...
			changeFace(index) {
				...
				uni.chooseMedia({
					...
					fail: (res) => {
						console.log("uni.chooseMedia调用失败：", res)
					}
				});
			},
