		...
		<div v-if="!wechatLogin">
			...
			<div class="flex" v-show="current != 1">
				...
				<div class="tips">
					...
					<span @click="navigateToPrivacys('USER_AGREEMENT')">《用户协议》</span>
				</div>
			</div>
			...
			<!-- <div v-if="current != 1" class="user-password-tips" @click="navigateToRegister">
				注册
			</div> -->
		<!-- 	<div v-if="current != 1" class="user-password-tips" @click="navigateToPassword">
				忘记密码
			</div> -->
			<!-- 循环出当前可使用的第三方登录模式 -->
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// 	content: '本程序只是杭师大学生内部使用，获取学号只用于身份验证，方便登录，修改密码',
		...
		methods: {
			...
			goOpenidLogin(clientType) {
				...
				uni.getStorageSync("unionId") ? (params.token.unionId = uni.getStorageSync("unionId")) : delete params
					.token;
			...
			navigateToPassword() {
				uni.navigateTo({
					url: `/pages/mine/set/securityCenter/forgetPassword`,
				});
			},
			// 点击获取验证码
			start() {
