...
export default {
  ...
  // 写入热门搜索时间戳
  setHotWords(val,tenantId) {
    uni.setStorageSync(`hotWords${tenantId}`, val);
  },
  // 获取热门搜索时间戳
  getHotWords(tenantId) {
    return uni.getStorageSync(`hotWords${tenantId}`);
  },
  //写入 展示还是不展示
  setShow(val) {
  ...
  setMainTenant(val){
    uni.setStorageSync('mainTenant', val)
  },
  //获取租户
  getMainTenant(){
    return uni.getStorageSync('mainTenant');
  },
};
