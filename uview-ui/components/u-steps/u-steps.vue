	...
	<view class="">
		<view class="u-steps" :style="{	flexDirection: direction}">
			<view class="u-steps__item" :class="['u-steps__item--' + direction]" v-for="(item, index) in list"
				:key="index" :style="{ padding: '0 '+ lineWidth[index] + 'rpx' }">
				...
				<view class="u-steps__item__dot" v-if="mode == 'dot'"
					:style="{ 	backgroundColor: index <= current ? activeColor : unActiveColor }"></view>
				<text :style="{ color: index <= current ? activeColor : unActiveColor,}"
...
<script>
	...
	export default {
		...
		watch: {
			list: 'calculateLineWidth'
		},
		methods: {
			calculateLineWidth() {
				this.lineWidth = this.list.map((item, index) => {
					return 85 * (index + 1) / this.list.length;
				});
			}
		},
	};
